...
class DNCNN(object):
    ...
    def network(self):
        ...
        with tf.name_scope('conv' + str(layer_count)):
            ...
            conv = tf.nn.conv2d(conv, weights, strides=[1, 1, 1, 1], padding='SAME')
            output = tf.add(self.images, conv, name='output')
    ...
    def train(self):
        ...
        '''
        self.save_model(model_name='model_save_while_train.pb')
    ...
    def test(self):
        ...
        writer = tf.summary.FileWriter('log_new')
        writer.add_graph(self.sess.graph)
        writer.flush()

        self.save_model(model_name = 'model_save_while_test.pb')
        ...
        for i in range(len(GT_file_list)):
            ...
            result = self.pred.eval({self.images:test_img})
        ...
        print('Average Time : ', duration / len(GT_file_list), 's')
    ...
    def save_model(self, model_name):
        ...
        frozen_graph = tf.graph_util.convert_variables_to_constants(self.sess, graphdef, INPUT_NAMES + OUTPUT_NAMES)
